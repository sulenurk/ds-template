...
def learn_model(df, target_col, params=None, search=True, cv=5, scoring='recall', random_state=42):
    """Train Support Vector Classifier with optional hyperparameter tuning"""
...
def apply_model(df, target_col, model_info: Dict) -> pd.DataFrame:
    """Apply SVC to data and return predictions + probabilities"""
...
def evaluate_model(df: pd.DataFrame, target_col: str):
    """Evaluate SVC predictions with metrics and probability-based plots"""
...
def explain_model(model, X_train, X_test, top_n_features=10, sample_index=None, index_feature=False, save_path=None):
    """Explain SVC using SHAP with kernel-based approximation"""
    shap_vals = sp.shap_values(model, X_train, X_test, model_type='kernel')
...
def save_model(model_info: Dict, filepath: str):
    """Save trained SVC model and metadata"""
    joblib.dump(model_info, filepath)
    print(f"Model saved to {filepath}")

# === 6. LOAD ===
def load_model(filepath: str) -> Dict:
    """Load trained SVC model and metadata"""
