...
def apply_model(df, target_col, model_info: Dict) -> pd.DataFrame:
    """Apply Random Forest to data and return predictions + probabilities"""
...
def evaluate_model(df: pd.DataFrame, target_col: str):
    """Evaluate classification predictions with metrics and probability-based plots"""
    ...
    if pred_proba is not None:
        ...
        print("Note: Probability-based plots skipped (no probabilities found).")

# === 4. EXPLAIN ===
def explain_model(model, X_train, X_test, top_n_features=10, sample_index=None, index_feature=False, save_path=None):
    """Use SHAP to explain Random Forest (tree-based)"""
...
def load_model(filepath: str) -> Dict:
    """Load model and its metadata from file"""
