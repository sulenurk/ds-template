...
def apply_model(df: pd.DataFrame, model_info: Dict, columns: Optional[List[str]] = None) -> pd.DataFrame:
    """Apply model to data and return predictions + probabilities, reinserting dropped columns if any."""
...
def evaluate_model(df: pd.DataFrame, target_col: str):
    """Evaluate XGBoost predictions with classification and probability metrics"""
